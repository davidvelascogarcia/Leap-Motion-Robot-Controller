...
/* 
 ...
 ROBOT PROTOTIPO CONTROL VÍA COORDENADAS CARTESIANAS CON INCLINACIÓN DE LA MANO VERTICAL Y HORIZONTAL, GARRA Y CONTROL POR VOZ, ASÍ COMO NOTIFICACIONES POR VOZ
...
namespace WindowsFormsApplication1//Namespace de la windows form
{
    public partial class Form1 : Form, ILeapEventDelegate//Uso de la clase publica de eventos del Leap Motion
    {
        ...
        string com4 = "COM7";//String para almacenar el pueto serie por l cual enviar los parametros
        public void detectHandPosition(Leap.Frame frame)//Función de detección de la posición de la mano
        {
            ...
            foreach (Hand hand in allHands)//Para cada muestra
            {
               ...
               if (grab == 1)//Si la pinza es 10, es decir esta cerrada
                {
              ...
              if (garraconversion == 1)//Si el resultado es mayor o igual a 7, mayoritariamente cerrada, asignamos los grados de cierre por reducción de errores
                {
                    garra = 140;//Asignamos grados
                }
                if (garraconversion == 0)//Si el resultado es menor  a 7, mayoritariamente abrierta, asignamos los grados de apertura por reducción de errores
                {
                    garra = 90;//Asignamos grados
                }
                ...
                /*if ((int)grab == 1)
               {
                ...
                coorx =(90-(((2)*(bb))/5));//Interpolacion de coordenadas del eje X
                coory= ((27) + (((9) * (cc)) / 200));//Interpolacion de coordenadas del eje Y
                coorz = ((425 / 5) - (((1) * (aa)) / 10));//Interpolacion de coordenadas del eje Z
                if (coorx > 150)
                {
                    coorx = 150;
                }
                ...
                int l = 0;//Control de notificacion de seguridad
                int m = 0;//Control de notificacion de seguridad
                int n = 0;//Control de notificacion de seguridad
                if (coorx < (20))
                {
                    l++;
                    coorx = 20;//Limitamos posición dos veces, en el main y en la API arduino
                    if (l == 3)//Notificación de seguridad a los 3 intentos
                    {
                        SpeechSynthesizer synth = new SpeechSynthesizer();
                ...
                {
                    l++;
                    coorx = 160;//Limitamos posición dos veces, en el main y en la API arduino
                    if (l == 3)//Notificación de seguridad a los 3 intentos
                    {
                  ...
                  if (coory < (30))
                   {
                    m++;
                    coory = 30;//Limitamos posición dos veces, en el main y en la API arduino
                    if (m == 3)//Notificación de seguridad a los 3 intentos
                    {
                        //Se instancia un nuevo objeto de sintesis de voz, selecciona el dispositivo de audio por defecto
                        //Se muestra por audio la sintesis del texto de notificacion preprogramado
                        SpeechSynthesizer synth = new SpeechSynthesizer();
                   ...
                   {
                    m++;
                    coory = 50;//Limitamos posición dos veces, en el main y en la API arduino
                    if (m == 3)//Notificación de seguridad a los 3 intentos
                    {
                        //Se instancia un nuevo objeto de sintesis de voz, selecciona el dispositivo de audio por defecto
                        //Se muestra por audio la sintesis del texto de notificacion preprogramado
                  ...
                  {
                    n++;
                    coorz = 30;//Limitamos posición dos veces, en el main y en la API arduino
                    if (n == 3)//Notificación de seguridad a los 3 intentos
                    {
                        //Se instancia un nuevo objeto de sintesis de voz, selecciona el dispositivo de audio por defecto
                        //Se muestra por audio la sintesis del texto de notificacion preprogramado
                        SpeechSynthesizer synth = new SpeechSynthesizer();
                  ...
                  {
                    n++;
                    coorz = 70;//Limitamos posición dos veces, en el main y en la API arduino
                    if (n == 3)//Notificación de seguridad a los 3 intentos
                    {
                        //Se instancia un nuevo objeto de sintesis de voz, selecciona el dispositivo de audio por defecto
                        //Se muestra por audio la sintesis del texto de notificacion preprogramado
                ...
                cartesianas = coorx + "," + coory + "," + coorz + "," + garra + "," + inclivert + "," + inclihor;
        ...
        private void timer1_Tick(object sender, EventArgs e)//Temprizador de la aplicación
        {
            ...
            if (i == 0)//Condición para ejecutar solo al inicio una vez
            {
                ...
                synth.Speak("El módulo de control de robot prototipo vía coordenadas cartesianas ha sido iniciado correctamente");
