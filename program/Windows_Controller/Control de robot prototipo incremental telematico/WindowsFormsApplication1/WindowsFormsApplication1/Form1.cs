...
/* 
 ...
 ROBOT PROTOTIPO CONTROL VÍA INCREMENTAL TELEMATICO CON INCLINACIÓN DE LA MANO VERTICAL Y HORIZONTAL, GARRA Y CONTROL POR VOZ, ASÍ COMO NOTIFICACIONES POR VOZ
...
namespace WindowsFormsApplication1//Namespace de la windows form
{
    public partial class Form1 : Form, ILeapEventDelegate//Uso de la clase publica de eventos del Leap Motion
    {
        ...
        string x;//Puesta a cero en control incremental
        string y;//Puesta a cero en control incremental
        string z;//Puesta a cero en control incremental
        ...
        public void detectHandPosition(Leap.Frame frame)//Función de detección de la posición de la mano
        {
            ...
            foreach (Hand hand in allHands)//Para cada muestra
            {
                ...
                x="0";//Puesta a cero en control incremental
                y="0";//Puesta a cero en control incremental
                z="0";//Puesta a cero en control incremental
              ...
              if (garraconversion >= 7)//Si el resultado es mayor o igual a 7, mayoritariamente cerrada, asignamos los grados de cierre por reducción de errores
                {
                    garra = 140;//Asignamos grados
                }
                if (garraconversion < 7)//Si el resultado es menor  a 7, mayoritariamente abrierta, asignamos los grados de apertura por reducción de errores
                {
                ...
                inclivert = ((64) - (((1) * (convver)) / 1));//Interpolacion de coordenadas de inclinacion vertical
                inclihor = ((60) - (((20) * (convver2)) / 1));//Interpolacion de coordenadas de inclinación horizontal
                ...
                if (i == 0)//Si se cumple la condición de analisis
                {
                    ...
                    System.IO.File.WriteAllText(@"C:\coordenadas.txt", coordenadasarduino);
        ...
        private void timer1_Tick(object sender, EventArgs e)//Temprizador de la aplicación
        {
            ...
            if (i == 0)//Condición para ejecutar solo al inicio una vez
            {
                ...
                synth.Speak("El módulo de control de robot prototipo vía incremental telematico ha sido iniciado correctamente");
        ...
        public void lector(object sender, SpeechRecognizedEventArgs e)//Función de reconocimento de palabras
        {
            foreach (RecognizedWordUnit palabra in e.Result.Words)//Analisis para cada palabra recibida
            {
                ...
                if (reconoce == "derecha")
                {
                    ...
                    cartesianas = coorx + "," + coory + "," + coorz + "," + garra + "," + inclivert + "," + inclihor;
                    x = "1";//Asignamos valor de ir a la izquierda
                ...
                if (reconoce == "izquierda")
                {
                    ...
                    x = "3";//Asignar valor de ir a la derecha
                    string coordenadasarduino = x + "," + y + "," + z;//Volcamos el valor concatenando sobre la variable, con los separadores ','
                    System.IO.File.WriteAllText(@"C:\coordenadas.txt", coordenadasarduino);
                }
                if (reconoce == "atrás")
                ...
                if (reconoce == "adelante")
                {
                    ...
                    z = "9";//Asignamos valor de adelante
                    string coordenadasarduino = x + "," + y + "," + z;//Volcamos el valor concatenando sobre la variable, con los separadores ','
                    System.IO.File.WriteAllText(@"C:\coordenadas.txt", coordenadasarduino);
                }
